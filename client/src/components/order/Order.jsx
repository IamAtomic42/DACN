...
import momo from "../../common/assets/momo.jpg";
...
export function Order(props) {
  ...
  const openPaymentMoMo = async () => {
    clientAxios
      .post("/order/momo", { listOrder: listOrder, payment: "paymentMOMO" })
      .then((res) => {
    ...
    <>
      ...
      {isShowMomo ? (
        <Modal
          ...
          aria-describedby="modal-modal-description"
        >
          <Card className="relative w-[40%] bg-blue-gray-50 opacity-90">
            <div className="w-full h-auto">
              <CloseIcon
                color="primary"
                className=" mb-2 cursor-pointer mr-4 mt-4  float-right hover:text-cyan-400"
                onClick={openMomo}
              />
            </div>
            <CardHeader className="w-auto mt-3 flex justify-center align-middle">
              <Typography
                variant="h4"
                className="text-black mt-[20px] mb-[35px]"
              >
                Tổng tiền: {formatPrice(totalPrice)}{" "}
              </Typography>
            </CardHeader>
            <CardBody
              className="p-0 h-[100%] ml-[8%] mt-14 w-[600px] object-cover text-center  overflow-y: scroll"
              id="modal-modal-description"
            >
              <img
                ...
                alt="image"
              />
              <Typography variant="h4" className="text-black mt-[20px]">
                <CrisisAlertIcon color="error" /> Hãy lưu ảnh thanh toán lại nhé
                !
              </Typography>
            </CardBody>
            <CardFooter className="mt-12 p-0 ">
